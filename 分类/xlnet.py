...
# ====================== 2. XLNet模型（接口与所有模型完全一致）=====================
class XLNetTextCls(nn.Module):
    def __init__(self, num_classes, dropout=0.1):
        super().__init__()
        from transformers import AutoModelForSequenceClassification
        # 加载预训练XLNet模型（效果接近MacBERT）
        self.xlnet = AutoModelForSequenceClassification.from_pretrained(
            "hfl/chinese-xlnet-base",
            num_labels=num_classes
        )

        # 冻结前10层，只训练最后2层（加快训练速度，效果几乎不变）
        for param in list(self.xlnet.parameters())[:-4]:
...
def test_model(model, test_loader, criterion, device, id2label, save_report=True):
    ...
    # 保存报告和混淆矩阵
    if save_report:
        report_path = os.path.join(CODE_ROOT, "xlnet_test_report.txt")
        ...
        plt.xlabel("预测标签")
        plt.ylabel("真实标签")
        plt.title("XLNet测试集混淆矩阵")
        cm_path = os.path.join(CODE_ROOT, "xlnet_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    # 初始化XLNet模型（接口与所有模型完全一致）
    print("\n初始化XLNet文本分类模型...")
    model = XLNetTextCls(num_classes, DROPOUT).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_acc > best_dev_acc:
            ...
            }, os.path.join(CODE_ROOT, "best_xlnet_cls_model.pth"))
            print("\n✅ 保存最佳XLNet模型")
    ...
    if test_loader:
        ...
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_xlnet_cls_model.pth"))
