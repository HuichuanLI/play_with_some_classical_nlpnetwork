...
def test_model(model, test_loader, criterion, device, id2label, save_report=True):
    ...
    # 保存报告和混淆矩阵
    if save_report:
        report_path = os.path.join(CODE_ROOT, "gru_test_report.txt")
        ...
        plt.xlabel("预测标签")
        plt.ylabel("真实标签")
        plt.title("GRU测试集混淆矩阵")
        cm_path = os.path.join(CODE_ROOT, "gru_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_acc > best_dev_acc:
            ...
            }, os.path.join(CODE_ROOT, "best_gru_cls_model.pth"))
            print("\n✅ 保存最佳模型")
    ...
    if test_loader:
        ...
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_gru_cls_model.pth"))
