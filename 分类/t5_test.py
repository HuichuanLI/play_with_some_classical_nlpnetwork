...
def load_data(train_path, dev_path=None, test_path=None, min_freq=1, test_size=0.2):
    ...
    # 加载T5中文分词器（原生支持pad_token）
    print("\n加载T5中文分词器...")
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained("Langboat/mengzi-t5-base")
    print(f"✅ T5分词器加载完成，词汇表大小：{len(tokenizer)}")
...
# ====================== 2. T5模型（接口与所有模型完全一致）=====================
# ====================== 2. T5模型（只使用编码器，彻底解决decoder问题）=====================
class T5TextCls(nn.Module):
    def __init__(self, num_classes, dropout=0.1):
        super().__init__()
        from transformers import T5EncoderModel  # 只加载编码器！
        # 只加载T5的编码器部分，不加载解码器
        self.t5_encoder = T5EncoderModel.from_pretrained("langboat/mengzi-t5-base")

        # 冻结前10层，只训练最后2层（加快训练速度）
        for param in list(self.t5_encoder.parameters())[:-4]:
            param.requires_grad = False

        # 分类头
        self.dropout = nn.Dropout(dropout)
        self.fc = nn.Linear(self.t5_encoder.config.hidden_size, num_classes)
...
def test_model(model, test_loader, criterion, device, id2label, save_report=True):
    ...
    # 保存报告和混淆矩阵
    if save_report:
        report_path = os.path.join(CODE_ROOT, "t5_test_report.txt")
        ...
        plt.ylabel("真实标签")
        plt.title("T5测试集混淆矩阵")
        cm_path = os.path.join(CODE_ROOT, "t5_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    # 初始化T5模型（接口与所有模型完全一致）
    print("\n初始化T5文本分类模型...")
    model = T5TextCls(num_classes, DROPOUT).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_acc > best_dev_acc:
            ...
            }, os.path.join(CODE_ROOT, "best_t5_cls_model.pth"))
            print("\n✅ 保存最佳T5模型")
    ...
    if test_loader:
        ...
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_t5_cls_model.pth"), weights_only=False)
    ...
    # 单句预测（接口完全不变）
    def predict(text):
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_t5_cls_model.pth"))
