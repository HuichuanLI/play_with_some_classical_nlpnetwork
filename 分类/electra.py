...
# ====================== 2. ELECTRA模型（接口与GRU/MacBERT完全一致）=====================
class ELECTRATextCls(nn.Module):
    def __init__(self, num_classes, dropout=0.1):
        super().__init__()
        from transformers import AutoModelForSequenceClassification
        # 加载预训练ELECTRA模型（中文效果最好的轻量级模型之一）
        ...
        for param in list(self.electra.parameters())[:-4]:
            param.requires_grad = False

        self.dropout = nn.Dropout(dropout)

    def forward(self, input_ids, attention_mask=None):
        # 接口与GRU/MacBERT完全一致：输入张量，输出logits
...
def test_model(model, test_loader, criterion, device, id2label, save_report=True):
    ...
    # 保存报告和混淆矩阵
    if save_report:
        report_path = os.path.join(CODE_ROOT, "electra_test_report.txt")
        ...
        plt.xlabel("预测标签")
        plt.ylabel("真实标签")
        plt.title("ELECTRA测试集混淆矩阵")
        cm_path = os.path.join(CODE_ROOT, "electra_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    # 初始化ELECTRA模型（接口与GRU/MacBERT完全一致）
    print("\n初始化ELECTRA文本分类模型...")
    model = ELECTRATextCls(num_classes, DROPOUT).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_acc > best_dev_acc:
            ...
            }, os.path.join(CODE_ROOT, "best_electra_cls_model.pth"))
            print("\n✅ 保存最佳ELECTRA模型")
    ...
    if test_loader:
        ...
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_electra_cls_model.pth"))
