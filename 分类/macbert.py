...
def load_data(train_path, dev_path=None, test_path=None, min_freq=1, test_size=0.2):
    ...
    # 加载MacBERT中文分词器（原生支持pad_token）
    print("\n加载MacBERT中文分词器...")
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained("hfl/chinese-macbert-base")
    print(f"✅ MacBERT分词器加载完成，词汇表大小：{len(tokenizer)}")
...
# ====================== 2. MacBERT模型（接口与GRU完全一致）=====================
class MacBERTTextCls(nn.Module):
    def __init__(self, num_classes, dropout=0.1):
        super().__init__()
        from transformers import AutoModelForSequenceClassification
        # 加载预训练MacBERT模型（中文效果最好的BERT变体）
        self.macbert = AutoModelForSequenceClassification.from_pretrained(
            "hfl/chinese-macbert-base",
            num_labels=num_classes
        )

        # 冻结前10层，只训练最后2层（加快训练速度，效果几乎不变）
        for param in list(self.macbert.parameters())[:-4]:
            param.requires_grad = False

        self.dropout = nn.Dropout(dropout)

    def forward(self, input_ids, attention_mask=None):
        # 接口与GRU完全一致：输入张量，输出logits
...
def test_model(model, test_loader, criterion, device, id2label, save_report=True):
    ...
    # 保存报告和混淆矩阵
    if save_report:
        report_path = os.path.join(CODE_ROOT, "macbert_test_report.txt")
        ...
        plt.xlabel("预测标签")
        plt.ylabel("真实标签")
        plt.title("MacBERT测试集混淆矩阵")
        cm_path = os.path.join(CODE_ROOT, "macbert_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    # 初始化MacBERT模型（接口与GRU完全一致）
    print("\n初始化MacBERT文本分类模型...")
    model = MacBERTTextCls(num_classes, DROPOUT).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_acc > best_dev_acc:
            ...
            }, os.path.join(CODE_ROOT, "best_macbert_cls_model.pth"))
            print("\n✅ 保存最佳MacBERT模型")
    ...
    if test_loader:
        ...
        checkpoint = torch.load(os.path.join(CODE_ROOT, "best_macbert_cls_model.pth"))
